...
class DatabaseService:
    ...
    def initialize_db(cls):
        ...
        with cls.get_connection() as conn:
            with conn.cursor() as cursor:
                ...
                # Create jobs table: links an analysis/rewrite job_id to the user who ran it,
                # so we can list a user's history even though Celery result backends are ephemeral.
                cursor.execute("""
                    CREATE TABLE IF NOT EXISTS jobs (
                        id VARCHAR(255) PRIMARY KEY,
                        user_id VARCHAR(255) REFERENCES users(id) ON DELETE CASCADE,
                        filename TEXT NOT NULL,
                        job_type VARCHAR(50) NOT NULL DEFAULT 'analyze',
                        created_at TIMESTAMPTZ NOT NULL DEFAULT NOW()
                    );
                """)
                cursor.execute("""
    ...
    @classmethod
    def create_user(cls, user_id: str, email: str, password_hash: str, full_name: str | None) -> None:
        with cls.get_connection() as conn:
            with conn.cursor() as cursor:
                cursor.execute(
                    """
    ...
    @classmethod
    def get_user_by_email(cls, email: str, include_password_hash: bool = False) -> dict | None:
        with cls.get_connection() as conn:
    ...
    @classmethod
    def create_job(cls, job_id: str, user_id: str | None, filename: str, job_type: str = "analyze") -> None:
        """Records a job so it can show up in a user's history. user_id may be None for anonymous uploads."""
        with cls.get_connection() as conn:
            with conn.cursor() as cursor:
                cursor.execute(
                    """
                    INSERT INTO jobs (id, user_id, filename, job_type)
                    VALUES (%s, %s, %s, %s)
                    ON CONFLICT (id) DO NOTHING;
                    """,
                    (job_id, user_id, filename, job_type),
                )
            conn.commit()

    @classmethod
    def get_jobs_for_user(cls, user_id: str, limit: int = 50) -> list[dict]:
        with cls.get_connection() as conn:
            with conn.cursor(cursor_factory=RealDictCursor) as cursor:
                cursor.execute(
                    """
                    SELECT id, filename, job_type, created_at
                    FROM jobs
                    WHERE user_id = %s
