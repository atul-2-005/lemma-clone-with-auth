...
@app.post(
    ...
    description="Registers a new user with email + password and returns a JWT access token.",
)
async def register(payload: UserCreate):
    try:
        user = AuthService.register_user(payload.email, payload.password, payload.full_name)
...
async def login(payload: UserLogin):
    ...
    access_token = create_access_token(user["id"])
    return Token(access_token=access_token, user=UserOut(**{k: v for k, v in user.items() if k != "password_hash"}))
...
@app.get(
    ...
    summary="Get the current user's job history",
    description="Returns the authenticated user's past analysis/rewrite jobs, with live status where available.",
)
async def get_my_jobs(current_user: dict = Depends(get_current_user)):
    jobs = DatabaseService.get_jobs_for_user(current_user["id"])
    enriched = []
    for job in jobs:
        ...
        enriched.append({**job, "status": job_status})
...
async def analyze_document_async(
    ...
    # Record the job against the user (if logged in) so it shows up in their history.
    # Anonymous uploads still work fine — user_id is just None.
    try:
        ...
        # Don't fail the upload just because history-tracking failed.
        import logging
        logging.getLogger(__name__).warning(f"Failed to record job history for {job_id}: {e}")
